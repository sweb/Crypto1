package fm.crypto.week1

object CryptoCall extends App {
  val otp = OTPEncryption()
  
  val target = "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"
  val c1 = ""
  val c2 = "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f"
  val c3 = "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb"
  val c4 = ""
  val c5 = "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070"
  val c6 = ""
  val c7 = ""
  val c8 = "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3"
  val c9 = ""
  val c10 = "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83"
  
  val ciphers = List(c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, target)
  
  val text = "We can see the point where the chip is unhappy if a wrong bit is sent and consumes more "
  val withCipher = c8
  //66396e89c9dbd8cc9874352acd6395102eafce78aa7fed28a07f6bc98d29c50b69b0339a19f8aa401a9c6d708f80c066c763fef0123148cdd8e802d05ba98777335daefcecd5
  val c3WithThe = otp.encrypt(text, otp.hexToString(withCipher))
  val allWithNew = ciphers.map( x => (otp.encrypt(otp.hexToString(x), otp.hexToString(c3WithThe))))
  val solvedTarget = otp.encrypt(otp.hexToString(target), otp.hexToString(c3WithThe))
  
  val combis = ciphers.map(x => (otp.encrypt(otp.hexToString(x), otp.hexToString(target))))
  
  val exercise = otp.encrypt("attack at dusk" ,otp.hexToString(otp.encrypt("attack at dawn", otp.hexToString("09e1c5f70a65ac519458e7e53f36"))))
  
  println(exercise)
  
  (32 to 122).toList.map(x => calcForChar(x))
  
//  println(c3WithThe)
//  println(otp.hexToListOfInts(c3WithThe))
//  println("-"*80)
//  
////  allWithNew.foreach(x => println(otp.hexToString(x).mkString("")))
//  println((Range(1, 11).zip(allWithNew)).map(x => "c" + x._1 + ": " +  otp.hexToString(x._2).mkString("")).mkString("\n"))
//  println("target: " + otp.hexToString(solvedTarget))
  
  def calcForChar(char: Int): Unit = {
  
  val combisWithSpace = combis.map(x => otp.encrypt(Character.toChars(char)(0).toString * (x.size/2), otp.hexToString(x)))
  
  val splitCombisWithSpace = combisWithSpace.map(x => otp.hexToListOfInts(x).sliding(1,1).toList.flatten).transpose
  
  val tt = splitCombisWithSpace.map(lists => lists.filter(x => (x >= 65 && x <= 122) || x==char).size > 8)
  
  println(Character.toChars(char)(0).toString + "(" + char + "):" + (Range(1,tt.size) zip tt).filter(_._2).map(x => x._1).mkString(","))
    
  }
  
  
}